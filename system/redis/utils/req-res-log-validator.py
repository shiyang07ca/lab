...
def process_file(docs, path):
    ...
    line_counter = [
        0
    ]  # A list with one integer: to force python to pass it by reference
    ...
    with io.StringIO(content) as fakefile:
        while True:
            try:
                ...
                print(
                    f"JSON decoder error while processing {path}:{line_counter[0]}: {err}"
                )
            ...
            try:
                jsonschema.validate(
                    instance=res.json, schema=req.schema, cls=schema_validator
                )
            except (
...
def fetch_schemas(cli, port, args, docs):
    ...
    cli_proc = subprocess.Popen(
        [cli, "-p", str(port), "--json", "command", "docs"], stdout=subprocess.PIPE
    )
